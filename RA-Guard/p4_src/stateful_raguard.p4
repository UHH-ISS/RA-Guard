...
// used to extract the length of the next extension header before parsing it
...
// struct for counting the occurance of each extension header
struct eh_counter_t {
    bit<8>     hbh;
    bit<8>     routing;
    bit<8>     fragment;
...
struct metadata_t {
    ...
    bool            is_learned;
    bool            is_forwarded;
    bit<8>          next_hdr;       // saves the last header parsing attempt
    eh_counter_t    eh_counter;     // saves the index of each last extension header parsing attempt
}

// additional packet information for the controller 
...
parser MyParser(packet_in packet,
    ...
    // Loop this state until every header in the chain is parsed
    ...
    // Loop this state until every NDP option is parsed
...
//------------------------------------------------------------------------------
    ...
    table multicast {
        ...
        size = 32;  // max number of ports
        default_action =  NoAction;

    }

//------------------------------------------------------------------------------
// Stateless RA-Guard filtering
//------------------------------------------------------------------------------
    ...
    table check_ra {
        ...
        size = 32; // max number of allowed router entries
        default_action =  blocking;
...
//------------------------------------------------------------------------------
    ...
    apply {
        if (hdr.ipv6.srcAddr[127:118] == LINK_LOCAL && hdr.ipv6.hop_limit == 255) {
            ...
            if (hdr.ndp_ra.isValid()){
                interface_state.apply();
                if (meta.is_forwarded == true) {
                    check_ra.apply();
                }
            }
        }
...
control MyEgress(inout headers_t hdr,
    ...
    apply {
        // Append additional information for controller directed packets 
        if (standard_metadata.egress_port == CPU_PORT) {
            hdr.packet_in.setValid();
            hdr.packet_in.ingress_port = standard_metadata.ingress_port;
            hdr.packet_in.is_blocked = (bit<1>) meta.is_blocked;
            hdr.packet_in.is_learned = (bit<1>) meta.is_learned;
            exit;
        }
        // Don't broadcast packets on the ingress port
